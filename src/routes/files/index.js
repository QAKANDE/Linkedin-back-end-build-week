...
const { readFile,createWriteStream} = require("fs-extra");
...
router.get("/csv/:username", async (req, res, next) => {
  try {
    ...
    const writeCSV = createWriteStream(join(__dirname,`../../../public/csv/${req.params.username}.csv`))

  experiences.pipe(json2csv).pipe(writeCSV)
  const path=join(__dirname,`../../../public/csv/${req.params.username}.csv`)
